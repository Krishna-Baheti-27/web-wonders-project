    ...
    <nav className="bg-lightgray text-darktext px-6 py-4 shadow-sm">
      ...
      >
        <div className="flex flex-col mt-4 space-y-2 text-lg font-sans text-darktext">
          <Link to="/" className="hover:text-cyan text-gray-700 transition" onClick={() => setIsOpen(false)}>Home</Link>
          <Link to="/live-map" className="hover:text-cyan text-gray-700 transition" onClick={() => setIsOpen(false)}>Live Map</Link>
          <Link to="/my-trips" className="hover:text-cyan text-gray-700 transition" onClick={() => setIsOpen(false)}>My Trips</Link>
          <Link to="/parcel" className="hover:text-cyan text-gray-700 transition" onClick={() => setIsOpen(false)}>Send a Parcel</Link>
          <Link to="/contact" className="hover:text-cyan text-gray-700 transition" onClick={() => setIsOpen(false)}>Contact</Link>
          <Link
            to={isAuthenticated ? "/user-logout" : "/user-login"}
            className="bg-royalblue text-white px-4 py-2 rounded hover:bg-cyan transition"
